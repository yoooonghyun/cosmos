...
| **Cosmos timeline — prompt context (in-bubble header)** | **on `bg-primary`** (`text-primary-foreground/80` labels) | **`#e9aee9`** | **The breadcrumb is now the STATIC HEADER section INSIDE the user-prompt box (header → `border-primary-foreground/20` divider → body), NOT a free-standing `bg-secondary` pill above the bubble. Labels `text-primary-foreground/80` (≈5.4:1), decoration `opacity-70`, NEVER `text-muted-foreground` on the pink (≈1.9:1 fail). Shares `max-w-chat-bubble` (D-11/D-14/D-18).** |
...
| **ContextMenu** | trigger/content/item (+ checkbox/radio/label/separator/sub) | The right-click / Shift+F10 menu (built on the unified `radix-ui` `ContextMenu` — NO extra install, D-19). Same chrome as Select: portalled `bg-popover` content at `z-overlay`/`shadow-overlay`/`duration-fast`, `rounded-md`; items + labels + shortcuts are all `text-caption` (12px)/`rounded-sm`/`gap-1.5`/symmetric `px-2 py-1.5` with `focus:bg-accent` hover. A floating menu runs ONE step DENSER than the rows it covers, so it sits TWO type steps tighter than Select (`text-caption` 12px vs Select's `text-body` 14px) — every site is the dense Home/Cosmos surface acting on `text-body-sm` rows (D-15). Used for Home's Pin/Unpin (tree rows + favorite strip tabs). A benign toggle → `variant="default"` items, NOT `destructive`. **Short 1-word toggle items (Pin/Unpin) carry NO leading icon** (a `size-4` glyph + `gap-1.5` ahead of a 1-word label reads strongly left-heavy though the `px-2` box is even; label-only is symmetric — D-19 round-2). |
...
|---|---|---|---|---|---|---|
| **User-prompt box (combined)** | `bg-primary` (#e9aee9) — the brand "my message" accent | `text-primary-foreground` (body) | `rounded-2xl rounded-br-sm`, `overflow-hidden` | right (`justify-end`) | `max-w-chat-bubble` | `text-body-sm` (body) |
| ↳ **context HEADER** (when context present) | inside the box, on `bg-primary` | labels `text-primary-foreground/80`, decoration `opacity-70` | — (static, no toggle) | top section, `px-3 pt-1.5 pb-1` | within box | `text-caption` |
| ↳ **divider** (when context present) | `border-t border-primary-foreground/20` | — | — | full-bleed under header | within box | — |
| ↳ **message BODY** (always) | on `bg-primary` | `text-primary-foreground` | — | below divider, `px-3 py-1.5` | within box | `text-body-sm` |
...
bubble carries the brand **`--primary`** accent because it is the user's OWN sent message — the
...
(header → divider → body, D-11/D-18), not a free-standing pill above it: a prompt and the screen it was
sent from read as ONE self-contained unit. The header stays QUIETER than the message — labels at
**`text-primary-foreground/80`** (≈5.4:1, AA), decoration at `opacity-70`, separated by a
**`border-primary-foreground/20`** inset divider — so **accent message vs quiet context** still holds:
...
pill and computes to ≈1.9:1 on the pink — see D-18.) The header is STATIC — no toggle, no collapse —
...
| **D-15** | **A renderer tree that surveys ANOTHER surface's items (the Cosmos panel-tab list) REUSES `FileTree`'s `role="tree"` roving-tabindex pattern + visual language — never a divergent keymap or a bespoke tree.** `ScrollArea` region (D-9); `h-7` `rounded-sm` rows; `text-body-sm` (§8); the VERBATIM FileTree keymap (↑/↓ + Home/End move focus, →/← expand·descend / collapse·ascend, Enter/Space activate); group header = `SURFACE_ICON[panelId]` glyph (D-10) + `RAIL_LABEL`; leaf row = ONE consistent lucide glyph (`AppWindow` for a tab) + label + Tooltip. The row states stay VISUALLY DISTINCT: hover `bg-accent`; roving focus = thin inset `--ring` (D-7); persistent **selection** = leading `--brand-accent` inset left bar + `bg-accent` + `font-medium` + `aria-selected` (D-4); a "live-but-inactive" marker (the source panel's ACTIVE tab) = a leading `--brand-accent` dot. **ADDITIVE pinned-state mark (cosmos-home-favorite-tabs-v1):** a source tab that ALREADY has a Home favorite is marked by `text-primary` on its leading icon (the brand accent, replacing the default `text-muted-foreground`) + a **bold label** (`font-medium`); a non-pinned row keeps its muted icon + normal weight. It is driven by the SAME `isPinned(panelId, tabId)` signal the Pin/Unpin ContextMenu uses (D-19) — ONE source of truth, never a second pin state — so a row's mark and its menu's Pin-vs-Unpin can never disagree; terminal rows are never pinned, so they never carry the mark. This is a NEW sanctioned `text-primary` (brand-accent) usage, a DELIBERATE state accent (not decoration) per explicit user choice, coherent with D-14's sparing-primary intent. Icon-color precedence: the row's hover/focus states recolor only the row BACKGROUND/ring (never the icon), so `text-primary` has no competing icon-color state — a selected/hovered pinned row still reads as pinned. Empty group = quiet `text-caption` "No open tabs" line (FileTree "Empty" idiom); no groups = one calm centered block; a malformed entry is skipped (warn+skip), never crashes. Reads synchronously (no fetch) ⇒ no loading skeleton. **ADDITIVE in-row inline rename (cosmos-tree-tab-rename-delete-v1):** a tree TAB row now also supports an IN-ROW inline edit reusing the `PanelTabStrip` rename idiom VERBATIM — the label `<span>` is swapped for a borderless `[field-sizing:content]` input (transparent bg, no box/ring, `text-body-sm`, `selection:bg-primary`), focused + select-once on entry, Enter/blur commit via the SAME pure `renameCommitDecision` (empty/whitespace reverts silently), Escape cancels; at most ONE row edits at a time, a vanished row ends the edit with no commit, and `stopPropagation` keeps typing from activating the row or firing the tree keymap. Entered from the row ContextMenu's **Rename** (D-19) or **F2** on the focused row (keyboard parity with the strip). The menu path defers the editor one tick so Radix's menu-close focus-restoration settles before the input mounts (otherwise the restored row focus blurs the input → premature commit). | The cross-panel tab tree needs THREE separable row states (hover ≠ keyboard-focus ≠ the tab chosen as context), one more than FileTree's two; codifying the pattern keeps every future "survey another surface" tree on the SAME keymap + visual language instead of re-inventing one, and pins the brand-accent active/selection affordance (D-4) so a selected/active row never reverts to a stray fill or blue. The pinned-state mark makes a pinned source visible AT the tree (not only via its right-click menu), reusing the single `isPinned` signal so the picker and the favorites state stay coherent. | `cosmos/PanelTabTree.tsx`, `fileExplorer/FileTree.tsx` (idiom source), design `.sdd/designs/cosmos-panel-tab-list-v1.md` |
...
| **D-18** | **A breadcrumb / caption rendered ON the `bg-primary` user-prompt fill draws from the `primary-foreground` FAMILY, never `text-muted-foreground`: meaningful LABELS at `text-primary-foreground/80` (≈5.4:1, AA for the 12px `text-caption`), `aria-hidden` DECORATION (glyphs/separators) at `opacity-70` relative (≈3:1, graphical AA); the message BODY stays full `text-primary-foreground` (≈8.5:1) so the body reads LOUD and the header QUIET. The in-bubble DIVIDER between the context header and the body is `border-t border-primary-foreground/20` — a quiet inset rule in the same foreground family — NEVER `border-border`/`border-border/60` (the dark line tuned for the muted ToolCallRow box). The combined box is `overflow-hidden` so the full-bleed divider clips to `rounded-2xl rounded-br-sm`. Null context → plain bubble: header + divider appear ONLY when context is present (the divider never appears alone).** | Moving the prompt-context breadcrumb off the `bg-secondary` (#3a3a3c) pill onto the `bg-primary` (#e9aee9 pastel pink) fill broke the legacy `text-muted-foreground` (#888888) glyph tone: on the pink it computes to ≈1.9:1 — below even the 3:1 graphical floor, a near-invisible smear (architect-flagged). Re-tone to the `primary-foreground` family (the body already proves #2e1065 on #e9aee9 = ≈8.5:1) fixes contrast AND preserves the D-11 "quiet context vs loud message" hierarchy via opacity steps of ONE foreground token, no new `--primary-foreground-muted` token for a one-surface need. The divider likewise can't borrow ToolCallRow's `border-border/60` (a foreign dark line on the pink); a low-alpha `primary-foreground` rule reads as part of the box. The shared, tone-neutral `PromptContextBreadcrumb` (labels `currentColor`, decoration `opacity-70`) lets the in-bubble header AND the kept `Badge variant="secondary"` standalone shell each set their own base family from ONE content source, so the two never fork. | §2 (in-bubble header row), §15 chat-surface canon, `cosmos/CosmosTimelineEntry.tsx` (`UserMessageBox`), `cosmos/PromptContextChip.tsx` (`PromptContextBreadcrumb`), `index.css` `--primary`/`--primary-foreground`; design `.sdd/designs/cosmos-context-message-combined-box-v1.md` |
| **D-19** | **The right-click / Shift+F10 menu is the shared shadcn/Radix `ContextMenu` primitive (`components/ui/context-menu.tsx`), built on the ALREADY-INSTALLED unified `radix-ui` package (`import { ContextMenu as ContextMenuPrimitive } from "radix-ui"` — it re-exports `@radix-ui/react-context-menu`; NO `@radix-ui/react-context-menu` install). It shares the `Select` menu's chrome (portalled `bg-popover`/`text-popover-foreground` content at `z-overlay`/`shadow-overlay`/`duration-fast`, `rounded-md`; items `rounded-sm`/`px-2 py-1.5` with `focus:bg-accent text-accent-foreground` hover, disabled = `opacity-50`), NEVER a hand-rolled menu — but its `Content` does NOT carry `scrollbar-hover-only` (see the gutter-asymmetry CORRECTION below; type step + icon rule also corrected there). Home's Pin/Unpin uses it on BOTH a `PanelTabTree` `TabRow` (state-reflective single item: `Pin` when unpinned / `Unpin` when pinned; terminal rows = DISABLED item + a `ContextMenuLabel` reason, FR-040) and a favorite STRIP tab (`Unpin`). Pin/Unpin is a BENIGN toggle → `variant="default"` items, NOT `destructive` (close `X` == unpin, reversible, no confirm). A FAVORITE strip tab reuses `PanelTabStrip` AS-IS plus an additive leading `SURFACE_ICON[source.panelId]` glyph (D-10) + the source label + closeable `X` (= unpin); it is distinguished from the default Cosmos tab by WHICH leading glyph it carries + the presence of the `X` (default = `closeable:false`, FR-011) — NO bespoke tab chrome (tab-strip canon / §14). **cosmos-home-keyboard-tab-nav-v1 (additive):** the pinned default "Cosmos" tab ALSO carries a leading glyph — its rail `SURFACE_ICON.cosmos` (D-10), the same source as the favorites' source-panel glyphs — so the strip reads uniformly (glyph + label on every tab); the two tabs differ by Cosmos-sparkle vs source-panel mark plus the favorite's `X`. A NON-closeable strip tab (the default, `closeable:false`) reserves the SAME width the close `X`'s `icon-xs` slot (`size-6`) would occupy via a trailing inert `aria-hidden` spacer, so a tab WITHOUT an `X` is not visually cramped beside closeable tabs — balanced as `leading glyph + label + reserved trailing slot`, matching closeable tabs' `leading + label + X`. This lives in `PanelTabStrip` (the one place), uses the `size-6`/`icon-xs` token (no raw px), and never affects closeable tabs (they keep the real `X`). A favorite's INLINE content is the source surface mounted through the SAME `ActiveTabSurface` host under the source panel's catalog (a LIVE mirror sharing `requestId`/`surfaceId`, no new contract, no extra cosmos spinner — D-8); its GONE-SOURCE + WAITING states reuse the `PanelTabTree` centered empty-block idiom (`flex-col items-center justify-center gap-2 py-8`, a `size-6 text-muted-foreground` `SURFACE_ICON` glyph, `text-foreground` headline + `text-caption text-muted-foreground` body) — calm, foundation-tokened, NEVER an Alert or `destructive`; the gone state carries an **Unpin** `Button variant="secondary" size="sm"` (an in-body inline control → `sm` is correct, D-13), and the favorite is NEVER auto-dropped (FR-031). CORRECTION (user feedback — a favorite is "literally a shortcut showing the source AS-IS"): a favorite tab is a SINGLE FULL-WIDTH pane — the cross-panel tab TREE + the timeline|tree split render ONLY on the default "Cosmos" tab (a favorite has NO tree, NO `ResizeDivider`) — and it shows the source view "as-is" INCLUDING the source panel's OWN floating Open Prompt: while a favorite is active Home HIDES its docked Cosmos composer (publishes a null `'cosmos'` config so the App-level `SharedComposer` renders nothing) and overlays the SOURCE panel's already-published composer (`useActiveComposerConfig(source.panelId)`) as a floating `PromptComposer` whose submit routes to the SOURCE target. NON-SECRET labels/surface only (FR-023). CORRECTION (menu-item density + icon-induced asymmetry, round-2): ContextMenu items are the DENSE menu treatment — items + labels + shortcuts are ALL `text-caption` (12px) + `gap-1.5` + symmetric `px-2 py-1.5` (no per-item margin; the `p-1` content inset is uniform, not a margin). A floating menu reads ONE step DENSER than the rows it covers, so items run TWO type steps tighter than Select (`text-caption` 12px vs Select's `text-body` 14px), not one — the first pass's `text-body-sm` (13px) still read oversized against the dense Home/Cosmos `text-body-sm` tree/tab rows it floats over (D-15), so the items dropped a further step to `text-caption`. SEPARATELY, the row's left/right whitespace read UNEVEN — the user's report is "오른쪽만 여백" (only the RIGHT has extra margin), and there are TWO causes, the PRIMARY one structural: **(P) `ContextMenuContent` carried `overflow-y-auto scrollbar-hover-only`, and `scrollbar-hover-only` sets `scrollbar-gutter: stable` (`index.css`), which RESERVES an 8px scrollbar gutter on the RIGHT edge ALWAYS — even for a short 2-item menu that never scrolls. So the content box's right inset = `p-1` + reserved gutter while top/left/bottom = just `p-1`, and every item's `focus:bg-accent` highlight stops short of the right edge by the gutter → the WHOLE dropdown reads right-heavy.** FIX (P): a transient overlay MENU must NOT use `scrollbar-hover-only`/`scrollbar-gutter: stable` — that utility is for PERSISTENT scroll regions where a bar appearing would shift content (D-9), not a momentary popover; a menu uses the platform OVERLAY scrollbar (no reserved gutter) so the inset is symmetric L/R, and `overflow-y-auto` alone still scrolls the (Radix-clamped, exceedingly rare) tall menu. Only `ContextMenuContent` carried it — `ContextMenuSubContent` is `overflow-hidden` (no gutter). **(S) secondary:** every Pin/Unpin item also rendered a LEADING `size-4` icon (`Pin`/`PinOff`) + `gap-1.5` ahead of a SHORT 1-word label, so the label's left gap (≈`px-2`8 + icon16 + gap6 ≈ 30px) dwarfed its right gap (`px-2` 8px) → an additional left-heaviness on those items. FIX (S): DROP the leading icon on the Pin/Unpin items (a 2-item reversible toggle needs no glyph) at the CALL SITES (`PanelTabTree` terminal-disabled/Pin/Unpin items + `CosmosPanel` favorite-strip Unpin item) — label-only items are symmetric within the `px-2` box; the primitive keeps its generic `[&_svg]` handling for menus that DO need icons. The first pass missed BOTH: it judged only the per-item CSS padding (symmetric) and never inspected the `Content`'s reserved scrollbar gutter (P, the dominant cause) or the icon gutter on a short label (S).** **EXTENSION (cosmos-tree-tab-rename-delete-v1):** the `PanelTabTree` row menu now carries `Pin/Unpin` → a `ContextMenuSeparator` → **Rename** + **Delete** (label-only dense items, no leading icon, same `text-caption` chrome). Rename begins the D-15 in-row inline edit; Delete is a BENIGN, reopenable close (it routes to the source panel's `close`, the same `X == unpin, no confirm` precedent) → `variant="default"`, explicitly NOT `destructive`. Both render only when the source panel published its reverse tab commands (`canEditTab`, FR-011) — a panel without them shows only Pin/Unpin. | The feature needed a menu primitive that did not exist; the plan flagged a possible `@radix-ui/react-context-menu` install, but the repo already depends on the unified `radix-ui` (`^1.6.0`) which re-exports `ContextMenu` (verified in `node_modules/radix-ui/dist/index.d.ts`), so adding it is a pure component-authoring step with no install — and using Radix ContextMenu gives native right-click + Shift+F10 keyboard for free (FR-003) instead of bespoke keyboard code. Pinning the primitive's styling to the Select-menu canon stops a future menu from forking onto a different surface; pinning the favorite tab to the existing strip (glyph-only distinction) and the inline mirror to the existing `ActiveTabSurface` host keeps Home reading as the SAME product as the source panels rather than re-skinning their surfaces, and routes the gone/waiting states onto the already-established calm empty-block idiom (not a red error). | `components/ui/context-menu.tsx` (authored), `cosmos/PanelTabTree.tsx` (tree-row trigger), `tabs/PanelTabStrip.tsx` (favorite glyph + per-tab menu slot), `cosmos/CosmosPanel.tsx` (strip favorites + content switch), `cosmos/FavoriteSurface.tsx` (inline mirror + gone/waiting states), `cosmos/favoriteCatalogHosts.tsx`; design `.sdd/designs/cosmos-home-favorite-tabs-v1.md`; spec/plan `cosmos-home-favorite-tabs-v1` |
| **D-20** | **A per-tab RANDOM "cosmos" glyph is a SECOND, sanctioned tab-glyph source DISTINCT from D-10's `SURFACE_ICON` — a per-tab DISTINGUISHER, not surface identity — bounded to the panel TAB STRIPS + the Cosmos Home TREE; it NEVER touches the rail/footer and NEVER replaces `SURFACE_ICON` anywhere.** Each newly minted tab of the four generative panels (Jira/Slack/Confluence/Calendar) + Terminal is assigned ONE random `iconId` from a FIXED 14-icon lucide set (`Rocket·Orbit·Satellite·SatelliteDish·Telescope·Atom·Star·MoonStar·Moon·Sun·SunMoon·Sparkle·Sparkles·Earth`) resolved via the pure registry (`src/shared/tabIcons.ts` vocabulary + `tabs/tabIconRegistry.tsx` id→component). It renders in the EXISTING tab-glyph treatment — `size-3.5`, `text-muted-foreground` → `group-data-[state=active]/tab:text-foreground`, `currentColor`, `aria-hidden` — adding NO new tokens and NO tab chrome (FR-005). TWO render seams share the one assigned id: (1) `PanelTabStrip`'s leading slot, where the per-tab `icon` (`LeadingIcon`) now PRECEDES the terminal default so a TERMINAL tab shows its random glyph and `SquareTerminal` is ONLY the fallback when a terminal tab has no icon (the OQ-2 REORDER) — spinner/error still win (FR-008); and (2) `PanelTabTree`'s leaf glyph = `tabIconComponent(tab.iconId) ?? AppWindow`, REPLACING the uniform `AppWindow` while PRESERVING the D-15 pinned-row `text-primary` tint + bold label + selected/focus/gone states (only the glyph SOURCE changes). FAVORITES are UNCHANGED — a Home favorite strip/tree mark KEEPS `SURFACE_ICON[source.panelId]` (D-19/D-15, the "shortcut to <panel>" read), and the generic Cosmos/Generated-UI default tab KEEPS `SURFACE_ICON.cosmos` (excluded from randomization). The id is a NON-SECRET bounded enum string persisted additively per tab (no schema bump). | A row of label-only tabs in a panel read as identical cells; a small per-tab space glyph makes open tabs distinguishable at a glance without widening the tab. A later reader could mistake a second tab-glyph for a D-10 violation, so this records that it is sanctioned and bounded (per-tab distinguisher ≠ rail/footer surface identity), and pins the terminal `SquareTerminal`-as-fallback reorder + the favorites-keep-`SURFACE_ICON` choice so they read correctly in the visual language. | `shared/tabIcons.ts`, `tabs/tabIconRegistry.tsx`, `tabs/PanelTabStrip.tsx` (leading-slot reorder), `cosmos/PanelTabTree.tsx` (leaf glyph), the four generative panels' + `TerminalPanel`'s `stripTabs`; spec/plan `cosmos-random-tab-icons-v1` |
