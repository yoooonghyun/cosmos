...
| SL-SCROLL-03 | A history list opens scrolled to the LATEST (bottom) message on initial load / channel switch; a top load-more (prepend) preserves position | visual + node-unit | `tests/visual/layout.visual.spec.ts`, `src/renderer/slackScrollToLatest.test.ts` |
