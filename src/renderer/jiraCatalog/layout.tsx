/**
 * jiraCatalog/layout — clamped Column/Row wrappers for the Jira custom A2UI catalog
 * (bug slack-generative-wrap-v1, Jira latent instance).
 *
 * The agent groups Jira lists/detail with the SDK standard-catalog `Column`/`Row`. Those
 * SDK containers render a `<div className="flex flex-col gap-4">` / `"flex flex-row gap-3">`
 * with NO `min-w-0`, so the flex box keeps `min-width: auto` and grows to its content's
 * intrinsic width — a long unbroken line then expands it past the panel and overflows
 * horizontally instead of wrapping (the leaf's `break-words` and the list root's `min-w-0`
 * cannot take effect once their containing block is already wider than the panel).
 *
 * We cannot change the third-party SDK div's className, so these wrappers render the SDK
 * `Column`/`Row` inside a block box carrying {@link JIRA_LAYOUT_CLAMP_CLASS}
...
import { JIRA_LAYOUT_CLAMP_CLASS } from './logic'
...
export function Column(props: React.ComponentProps<typeof SdkColumn>): React.JSX.Element {
  return (
    <div className={JIRA_LAYOUT_CLAMP_CLASS}>
      <SdkColumn {...props} />
    </div>
  )
}
