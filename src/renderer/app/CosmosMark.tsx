/**
 * CosmosMark — the cosmos symbol (4-point star/sparkle) as an inline SVG. Inlined rather
 * than an `<img>` so it is CSP-safe without a loader (`img-src 'self' data:` blocks remote
 * URLs) and the gradient id can be made per-instance.
 *
 * Two `tone`s share the ONE mark path so the brand glyph never forks:
 *  - `'pastel'` (default) — the brand pink→purple linear gradient
 *    (`assets/logo/cosmos-symbol-pastel.svg`); the rail/launch mark.
 *  - `'mono'` — a single neutral tone via `fill="currentColor"`, so the caller colors it by
 *    text token (e.g. `text-muted-foreground`). This is the small assistant-avatar mark in
 *    the Cosmos timeline (DESIGN.md §15 / D-17): one muted glyph, no gradient.
 *
 ...
 * leaving the visible mark transparent (mono skips the gradient entirely). The fixed
 * `width/height` are dropped (the caller sizes via `className`, e.g. `size-8`); the mark is
 * `aria-hidden` because the owning button/avatar carries the meaning (FR-013).
 */
...
export function CosmosMark({
  tone = 'pastel',
  ...props
}: SVGProps<SVGSVGElement> & { tone?: 'pastel' | 'mono' }): React.JSX.Element {
  ...
  return (
    ...
    >
      ...
      <g
        transform="translate(100,100) scale(1.42)"
        fill={mono ? 'currentColor' : `url(#${pastelId})`}
      >
