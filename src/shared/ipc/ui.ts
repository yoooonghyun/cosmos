...
export const UiChannel = {
  ...
  /**
   * M->R: the EARLY "UI generation has begun" begin-signal (ui-catalog-pull-spinner-
   * signal-v1, FR-004). Emitted when a render run pulls the component catalog
   * (`get_ui_catalog`), BEFORE any surface is composed — the renderer turns the
   * originating tab's "Generating…" spinner ON. NON-SECRET: carries only the render
   * `target`. Distinct from `ui:render` (which carries the finished surface).
   */
  GeneratingBegin: 'ui:generatingBegin'
} as const
...
/**
 * M->R. The EARLY "UI generation has begun" begin-signal (ui-catalog-pull-spinner-
 * signal-v1, FR-004). Pushed when a render run calls `get_ui_catalog` (over the bridge),
 * BEFORE any surface is composed — the renderer turns the originating tab's "Generating…"
 * spinner ON (gating it on this signal instead of optimistically at submit, FR-005).
 *
 * NON-SECRET by contract: carries ONLY the render `target`, no token/transcript/surface
 * content (FR-011). Validated warn-and-ignore at the main boundary (`validateUi
 * GeneratingBeginPayload`, FR-012); an invalid one is dropped (spinner defaults hidden).
 */
export interface UiGeneratingBeginPayload {
  /** Which panel this UI generation targets — the renderer routes the spinner by it. */
  target: UiRenderTarget
}
...
export interface UiApi {
  ...
  /**
   * M->R. Subscribe to the EARLY "UI generation has begun" begin-signal
   * (ui-catalog-pull-spinner-signal-v1, FR-004/FR-005). Fired when a render run pulls the
   * component catalog; the panel turns the originating tab's spinner ON. Returns an
   * unsubscribe fn so the panel can detach on unmount. NOTE: a NEW preload method — a full
   * `npm run dev` restart is required (HMR alone leaves it `not a function`).
   */
