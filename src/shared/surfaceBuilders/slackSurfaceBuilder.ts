/**
 * slackSurfaceBuilder (SHARED) — pure Slack → A2UI 0.9 SLACK-CUSTOM-catalog BOUND SHELL /
 * row composition (slack-generative-adapter-v1, FR-002/FR-003).
 *
 * Lives in `src/shared/` so both main and renderer can reuse these PURE, secret-free
 * helpers; the main `src/main/slack/slackSurfaceBuilder.ts` + `slackAdapter.ts` RE-EXPORT
 * them (single source of truth; main callers unchanged).
 *
 * The bound `ChannelList`/`MessageList`/`SearchResultList` shells carry `{path}` bindings
 * instead of literal row props; the shared AdapterDispatcher pushes fresh `updateDataModel`
 * on refresh / load-more (append). The row shapes (`slackChannelRow`/`slackMessageRow`/
 * `slackSearchRow`) are unchanged (FR-002/FR-004). Pure mapping: NO Slack API calls, no
 * IPC, no secrets (FR-018).
 */

import type { A2uiSurfaceUpdate } from '../ipc'
import type { SlackChannel, SlackMessage, SlackSearchMatch } from '../types/slack'
